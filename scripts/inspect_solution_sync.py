...
Usage:
  python scripts/inspect_solution_sync.py --audio public/Version-3/Day05/New_Day5Question01sol.mp3 --code "SELECT SUM(salary) AS total_payroll, AVG(salary) AS avg_salary FROM employees;"
  python scripts/inspect_solution_sync.py --day 5
  python scripts/inspect_solution_sync.py --day 5 --patch-engine
"""
...
def format_sql_multiline(sql: str) -> str:
    ...
    # If already multi-line, preserve structure
    if '\n' in sql:
        return sql
    ...
    # Split SELECT list if multiple columns
...
def split_sql_into_inspectable_segments(formatted_sql: str) -> List[str]:
    ...
    for line in lines:
        line_with_nl = line + '\n'
        # If line contains major clause + expression (e.g. "WHERE status = 'Shipped'")
    ...
    # Ensure the last segment doesn't have an extra trailing newline if not needed, but keeps semicolon
...
def align_sql_segments_to_audio(segments: List[str], words: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Common speech keyword mappings
    speech_aliases = {
        'sum': ['sum', 'some'],
        'avg': ['avg', 'average', 'vg'],
    ...
    for seg_idx, seg_text in enumerate(segments):
        ...
        # Search for first matching token in words from current_word_idx
        ...
        # Fallback estimation if token not directly identified
...
def main():
    ...
    if args.audio and args.code:
        ...
        print(f"Inspecting Day {args.day} practice questions...")
        # Read day file to extract questions
        with open(day_file, 'r', encoding='utf-8') as f:
            content = f.read()
            
        # Match practice questions
        pq_matches = re.findall(r'id:\s*(\d+).*?solution:\s*`([^`]+)`', content, re.DOTALL)
        if not pq_matches:
            pq_matches = re.findall(r"id:\s*(\d+).*?ref:\s*['\"]([^'\"]+)['\"]", content, re.DOTALL)
            
        day_dir = f"public/Version-3/Day{str(args.day).zfill(2)}"
        for q_id_str, sql in pq_matches:
            q_id = int(q_id_str)
            audio_file = f"{day_dir}/New_Day{args.day}Question{str(q_id).zfill(2)}sol.mp3"
            if os.path.exists(audio_file):
                report = inspect_solution(audio_file, sql, q_id)
                print_inspection_report(report)
            else:
                print(f"Skipping Q{q_id}: Audio file {audio_file} not found")
    else:
        parser.print_help()
