...
LIMIT  5 OFFSET 5;</code></pre>
  ...
  "testQuestions": [
    { "id": 1, "prompt": "Write a query to pull every column from the <code>employees</code> table — no filtering, no limits.", "ref": "SELECT * FROM employees;" },
    { "id": 2, "prompt": "A payroll report only needs <code>first_name</code> and <code>salary</code> from <code>employees</code>. Write the SELECT statement.", "ref": "SELECT first_name, salary FROM employees;" },
    { "id": 3, "prompt": "Your team lead asks: &quot;What are all the distinct job titles we have?&quot; Query the <code>employees</code> table for unique <code>job_title</code> values.", "ref": "SELECT DISTINCT job_title FROM employees;" },
    { "id": 4, "prompt": "HR wants a salary ladder — retrieve <code>first_name</code>, <code>last_name</code>, and <code>salary</code> from <code>employees</code>, sorted from lowest to highest salary.", "ref": "SELECT first_name, last_name, salary FROM employees ORDER BY salary ASC;" },
    { "id": 5, "prompt": "The CEO asks for the top 3 earners. Return <code>first_name</code> and <code>salary</code> from <code>employees</code>, highest paid first, limited to 3 rows.", "ref": "SELECT first_name, salary FROM employees ORDER BY salary DESC LIMIT 3;" },
    { "id": 6, "prompt": "A product listing page needs just the product name and price. From <code>products</code>, retrieve <code>name</code> and <code>unit_price</code>, renaming <code>unit_price</code> as <code>price</code>.", "ref": "SELECT name, unit_price AS price FROM products;" },
    { "id": 7, "prompt": "You need a quick peek at the <code>orders</code> table. Retrieve all columns but cap the result at 5 rows.", "ref": "SELECT * FROM orders LIMIT 5;" },
    { "id": 8, "prompt": "Marketing wants to know which regions are represented in <code>customers</code>. Return unique <code>region</code> values, sorted A to Z.", "ref": "SELECT DISTINCT region FROM customers ORDER BY region ASC;" },
    { "id": 9, "prompt": "Generate a department roster: retrieve <code>first_name</code>, <code>last_name</code>, and <code>department_id</code> from <code>employees</code>, ordered by department first, then last name alphabetically.", "ref": "SELECT first_name, last_name, department_id FROM employees ORDER BY department_id ASC, last_name ASC;" },
    { "id": 10, "prompt": "Finance wants to see profit-per-product. From <code>products</code>, retrieve <code>product_id</code>, <code>name</code>, and a computed column <code>profit</code> defined as <code>unit_price - cost_price</code>.", "ref": "SELECT product_id, name, unit_price - cost_price AS profit FROM products;" },
    { "id": 11, "prompt": "Warehouse needs to know stock levels. Retrieve all columns from <code>products</code>, ranked by <code>stock_qty</code> from highest to lowest.", "ref": "SELECT * FROM products ORDER BY stock_qty DESC;" },
    { "id": 12, "prompt": "Build a monthly-pay leaderboard: from <code>employees</code>, retrieve <code>first_name</code> and <code>salary</code> aliased as <code>monthly_pay</code>, sorted highest to lowest using the alias.", "ref": "SELECT first_name, salary AS monthly_pay FROM employees ORDER BY monthly_pay DESC;" },
    { "id": 13, "prompt": "How many distinct departments exist? Start by listing all unique <code>department_id</code> values from <code>employees</code>.", "ref": "SELECT DISTINCT department_id FROM employees;" },
    { "id": 14, "prompt": "Customer success wants to track signup history. Retrieve all columns from <code>customers</code>, sorted by <code>signup_date</code> oldest-first.", "ref": "SELECT * FROM customers ORDER BY signup_date ASC;" },
    { "id": 15, "prompt": "An operations dashboard shows the 5 most recent orders. From <code>orders</code>, retrieve <code>order_id</code>, <code>total_amount</code>, and <code>status</code>, newest first, limited to 5 rows.", "ref": "SELECT order_id, total_amount, status FROM orders ORDER BY order_date DESC LIMIT 5;" },
    { "id": 16, "prompt": "Produce an alphabetical employee directory. From <code>employees</code>, retrieve <code>first_name</code>, <code>last_name</code>, and <code>email</code>, sorted by <code>last_name</code> A to Z.", "ref": "SELECT first_name, last_name, email FROM employees ORDER BY last_name ASC;" },
    { "id": 17, "prompt": "What unique combinations of <code>department_id</code> and <code>is_active</code> status exist in <code>employees</code>? Write a DISTINCT query to find out.", "ref": "SELECT DISTINCT department_id, is_active FROM employees;" },
    { "id": 18, "prompt": "The pricing team needs a margin analysis. From <code>products</code>, retrieve <code>name</code>, <code>unit_price</code>, <code>cost_price</code>, and a <code>margin_pct</code> column computed as <code>(unit_price - cost_price) * 100 / unit_price</code>.", "ref": "SELECT name, unit_price, cost_price, (unit_price - cost_price) * 100 / unit_price AS margin_pct FROM products;" },
    { "id": 19, "prompt": "You need to paginate through employee records. Fetch rows 6–10 from <code>employees</code> ordered by <code>employee_id</code> — use <code>LIMIT</code> with <code>OFFSET</code>.", "ref": "SELECT * FROM employees ORDER BY employee_id LIMIT 5 OFFSET 5;" },
    { "id": 20, "prompt": "Generate a regional customer list. From <code>customers</code>, retrieve <code>customer_id</code>, <code>first_name</code>, and <code>region</code> — sorted by region, then by first name within each region.", "ref": "SELECT customer_id, first_name, region FROM customers ORDER BY region ASC, first_name ASC;" },
    { "id": 21, "prompt": "Get a quick sample of the <code>order_items</code> table — all columns, first 5 rows only.", "ref": "SELECT * FROM order_items LIMIT 5;" },
    { "id": 22, "prompt": "Tax season: every employee owes 20% of their salary. From <code>employees</code>, show <code>first_name</code>, <code>salary</code>, and a computed <code>tax</code> column (<code>salary * 0.2</code>).", "ref": "SELECT first_name, salary, salary * 0.2 AS tax FROM employees;" },
    { "id": 23, "prompt": "What order statuses are currently in use? Query <code>orders</code> for all unique <code>status</code> values.", "ref": "SELECT DISTINCT status FROM orders;" },
    { "id": 24, "prompt": "Budget-conscious buyers want the cheapest products first. From <code>products</code>, retrieve <code>product_id</code>, <code>name</code>, and <code>unit_price</code>, sorted cheapest-first, limited to 3 items.", "ref": "SELECT product_id, name, unit_price FROM products ORDER BY unit_price ASC LIMIT 3;" },
    { "id": 25, "prompt": "Build a new-hire report. From <code>employees</code>, retrieve <code>employee_id</code>, <code>first_name</code>, <code>last_name</code>, and <code>hire_date</code>, ordered by <code>hire_date</code> descending (most recently hired first).", "ref": "SELECT employee_id, first_name, last_name, hire_date FROM employees ORDER BY hire_date DESC;" }
  ],
  "topics": [
