...
// ── COMPONENT 8: DESKTOP LAYOUT SCROLL LOCK & TOC TRACKING FIXES ──
runWhenReady(() => {
  // Prevent viewport/window scrolling on desktop (min-width: 769px)
  window.addEventListener('scroll', () => {
    if (window.innerWidth >= 769 && window.pageYOffset > 0) {
      window.scrollTo(0, 0);
    }
  });

  // Intercept TOC link clicks to scroll the .notebook container instead of the window
    ...
    if (link && window.innerWidth >= 769) {
      ...
      if (href && href.startsWith('#')) {
        ...
        if (targetEl && container) {
          ...
          container.scrollTo({
            top: relativeTop - 20, // Offset for aesthetics
            behavior: 'smooth'
          });
          // Update URL hash without triggering window scroll
  ...
  function scrollToHash() {
    ...
    if (hash && hash.startsWith('#') && window.innerWidth >= 769) {
  ...
  // Monitor internal scrolling on .notebook container for TOC active tracking on desktop
  const notebookContainer = document.getElementById('notebook');
  if (notebookContainer) {
    notebookContainer.addEventListener('scroll', () => {
      if (window.innerWidth >= 769 && typeof updateToc === 'function') {
        updateToc();
      }
    });
  }
